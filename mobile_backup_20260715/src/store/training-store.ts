...
export const useTrainingStore = create<TrainingState>()(
  persist(
    (set) => ({
      ...
      // le schede/lo storico dei clienti demo non devono apparire nell'app
      // normale (i clienti seed stessi sono gia' gated in client-store.ts).
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        ...
        // Purge dei seed gia' persistiti da versioni precedenti (vedi
        // client-store.ts per il razionale): rimuove SOLO schede/storico con
        // id seed noto, mai piani reali (id Supabase o creati dal coach).
        if (!DEMO_DATA_ENABLED && state) {
          const seedPlanIds = new Set(SEED_WORKOUT_PLANS.map((plan) => plan.id));
          const seedHistoryIds = new Set(SEED_PROGRESS_HISTORY.map((entry) => entry.id));
          useTrainingStore.setState((s) => ({
            workoutPlans: s.workoutPlans.filter((plan) => !seedPlanIds.has(plan.id)),
            progressHistory: s.progressHistory.filter((entry) => !seedHistoryIds.has(entry.id)),
          }));
        }
      },
    }
