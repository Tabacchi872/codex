...
import { AppButton, AppCard, AppScreen, AppTextField, BackHeader, UserAvatar } from '@/components/ui';
import { CoachOnlyNotice } from '@/components/coach-only-notice';
import { useAuthStore } from '@/store/auth-store';
import { useClientStore } from '@/store/client-store';
import { AppFontSize, AppRadius, AppSpacing, useAppTheme } from '@/theme';
import { CLIENT_STATUS_LABEL, type Client, type ClientAvatarPreset, type ClientStatus } from '@/types/client';
...
export default function NuovoClienteScreen() {
  ...
  const [avatarPreset, setAvatarPreset] = useState<ClientAvatarPreset>('neutral');
  ...
  return (
    <AppScreen>
      <BackHeader title="Nuovo cliente" fallbackHref="/clienti" />
      <AppCard style={styles.form}>
        ...
        <View style={styles.field}>
          <Text style={[styles.fieldLabel, { color: colors.inkSoft }]}>Avatar predefinito</Text>
          <View style={styles.avatarChoiceRow}>
            <UserAvatar firstName={firstName} lastName={lastName} preset={avatarPreset} size={52} />
            <View style={styles.chipsRow}>
              {AVATAR_OPTIONS.map((option) => {
                const active = option.value === avatarPreset;
                return (
                  <Pressable
                    key={option.value}
                    onPress={() => setAvatarPreset(option.value)}
                    style={[styles.chip, { backgroundColor: active ? colors.moss : 'transparent', borderColor: colors.moss }]}>
                    <Text style={[styles.chipLabel, { color: active ? colors.onMoss : colors.moss }]}>{option.label}</Text>
                  </Pressable>
                );
              })}
            </View>
