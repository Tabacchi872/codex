...
- `subscription_packages` (2026-07-12, esteso 2026-07-14 per RevenueCat): pacchetti acquistabili gestiti dal superadmin, distinti da `plans`/`coach_billing` (piano SaaS interno del coach) e da `subscriptions` (pacchetto sessioni creato dal singolo coach per il proprio cliente). `id uuid primary key`, `target_role text check in ('coach','client')`, `name`, `description`, `price numeric(10,2)` solo come riferimento interno, `currency default 'EUR'`, `duration_value integer`, `duration_unit text check in ('days','months')`, `max_clients integer null` (solo per `target_role='coach'`, vincolo check che lo forza a `null` per `target_role='client'`), `features jsonb default '[]'`, mapping store `revenuecat_entitlement_id`, `revenuecat_offering_id`, `android_product_id`, `ios_product_id`, `is_active`, `sort_order`, `created_at`, `updated_at`.
  `android_product_id` e `ios_product_id` sono univoci quando non null; `revenuecat_entitlement_id` non e' univoco per consentire pacchetti mensili/annuali con lo stesso entitlement. Per Google Play `android_product_id` puo' essere `subscription_id:base_plan_id`.
...
- `revenuecat_webhook_events` (2026-07-14): ledger idempotente degli eventi RevenueCat processati dalla Edge Function `revenuecat-webhook`. `event_id unique`, `event_type`, `app_user_id`, `product_id`, `entitlement_id`, `payload jsonb`, `processed`, `processing_error`, `received_at`, `processed_at`. RLS abilitata senza policy utente: accesso tramite service role della function.
...
- **Un solo abbonamento `active` per utente**: trigger `user_subscriptions_single_active` (`enforce_single_active_user_subscription()`) marca `canceled` ogni altra riga `active` dello stesso `user_id` quando una nuova riga diventa `active` — un pacchetto acquistato/rinnovato non accumula mai slot, il pacchetto attivo corrente determina sempre il limite corrente.
  Gli eventi RevenueCat non cancellano righe `payment_provider='superadmin_manual'`: le assegnazioni manuali restano protette e distinguibili.
- **Pacchetto effettivo**: funzione interna `public._effective_coach_subscription(p_coach_id)` con priorita' unica per tutto il backend: `superadmin_manual` active/non scaduto prima, poi `revenuecat` active/non scaduto, poi nessun pacchetto. Quando l'override manuale scade, un RevenueCat ancora valido torna effettivo automaticamente senza webhook nuovo.
