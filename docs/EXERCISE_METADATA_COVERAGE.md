...
**Aggiornamento 2026-07-27 (BUG-055 chiuso, poi riaperto e corretto di nuovo lo stesso giorno):** dopo la migration `20260807090000_fix_bug_055_auto_template_content.sql`, **18/18 template PASS**. I 3 FAIL descritti sotto (storico, al momento del seed di questo sotto-blocco) sono stati corretti sostituendo gli esercizi incompatibili. Una verifica semantica più rigorosa richiesta dall'utente ha poi trovato che 2 di quelle sostituzioni non preservavano davvero attrezzatura/funzione tecnica del template (validatore con due punti ciechi: testo attrezzatura mai controllato, `substitution_group` mai controllato) — corrette con la migration `20260808090000_fix_bug_055b_template_semantic_equipment.sql`, **18/18 confermato di nuovo con la regola corretta**. Dettaglio completo (tabella prima/dopo, criteri di sostituzione, causa del falso PASS, test) in `docs/BUG_055_TEMPLATE_FIX.md`. Il motore di revisione (2.3) può ora fidarsi del flag `auto_eligible` per tutti e 18 i template.
