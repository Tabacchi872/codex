...
## 24. Sotto-blocco 2.2 — seed metadati esercizio implementato (2026-07-27, migration `20260805090000_block2_exercise_metadata_seed.sql` + `20260806090000_block2_exercise_alternatives_gaps.sql`)

Dettaglio completo, tassonomia adottata, fonti, eccezioni, verifica dei 18 template e delle 13 categorie campionate: **`docs/EXERCISE_METADATA_COVERAGE.md`** (documento dedicato, come previsto dal compito). Sintesi:

- **96/96 esercizi del catalogo classificati** (100%), **78/78 esercizi usati dai template `auto_eligible` con metadati** (100%), **0 riferimenti orfani**.
- **Estensione tassonomia — solo 3 valori, necessità dimostrata**: `movement_pattern` esteso con `core_flexion`/`core_extension`/`core_rotation` (crunch/hyperextension/russian-twist sono movimenti dinamici, non tenute isometriche di resistenza come plank/pallof-press — forzarli in `core_anti_extension`/`core_anti_rotation` sarebbe stata una classificazione fattualmente sbagliata). Nessun'altra estensione ai CHECK del 2.1: attrezzatura/luogo/livello/categoria/gruppi muscolari riusano identiche le tassonomie già reali (equipment_tag a 3 livelli identico a `client_fitness_profile.equipment_level`, gruppi muscolari identici a `ExerciseMuscleGroupId`).
- **114 relazioni in `exercise_alternatives`**, tutte direzionali, curate (non generate meccanicamente per solo gruppo muscolare condiviso), con priorità/difficoltà relativa/motivo espliciti.
...
## 27. Sotto-blocchi 2.3-2.7 — implementati, testati, Blocco 2 completo (2026-07-28)

Dettaglio completo di ogni sotto-blocco in `docs/WORKLOG.md` (entry dedicate per data); bug trovati in `docs/BUGS.md` BUG-058/BUG-059; decisioni tecniche in `docs/DECISIONS.md`. Sintesi qui, per non duplicare quanto già scritto per esteso altrove:

- **2.3 — motore di revisione** (`check_cycle_review_eligibility`/`submit_monthly_checkin`/`run_cycle_review`, 17 passi): implementato e testato il 2026-07-28 (vedi Aggiornamento 43 in `docs/PROJECT_STATE.md`). Il ramo `pending_template`, non testato dal vivo in quella sessione, è stato verificato con successo in questa sessione con dati sintetici ed esclusioni cliente-specifiche (mai template/metadati condivisi disattivati) — 1 fix trovato (`metrics_snapshot` vuoto nel ramo "template svuotato dalle esclusioni").
- **2.4 — passaggio self-guided → coach**: nuovo trigger `_handle_client_coach_assignment()` su `coach_clients` (chiude il ciclo automatico a `replaced`, mai cancellato, mai tocca schede/check-in/storico). 1 fix (conflitto con l'indice unico "una sola review definitiva").
- **2.5 — RPC Superadmin**: 7 nuove RPC (`superadmin_list_auto_clients`, `superadmin_get_client_program_history`, `superadmin_resolve_safety_review`, `superadmin_resolve_pending_template`, `superadmin_replace_single_exercise`, `superadmin_force_cycle_decision`, `superadmin_cancel_pending_review`), tutte auditate in `superadmin_program_overrides` (schema già pronto dal 2.1). 4 fix trovati eseguendo le RPC live.
- **2.6 — schermate mobile**: `check-in-periodico.tsx` (nuova, multi-step), `AutoProgramCard` esteso a tutti gli 8 stati non-terminali, servizi/store dedicati. **BUG-058** trovato e corretto (drift di vocabolario mai propagato dal 2.1).
- **2.7 — campagna di test end-to-end finale**: account sintetici + **token REST reali** (login password, mai solo simulazione SQL del ruolo). Copertura: rivalidazione decisione+idempotenza di `run_cycle_review`; sicurezza (anon, grants su 8 RPC Superadmin, cross-client, payload manipolato via REST bypass RPC, RLS diretta); doppia chiamata concorrente reale (`Promise.all`); scadenza→rinnovo abbonamento end-to-end; risoluzione sicurezza Superadmin (`approve_continue`) end-to-end; cancellazione account con storico Blocco 2 reale via l'Edge Function di produzione `delete-account`; cleanup totale. **BUG-059** (6 fix, catena "blocco temporaneo vs decisione definitiva" in codice 2.3 già "chiuso") trovato e corretto interamente da questa campagna — mai dalla sola lettura del codice.
- **Non testato con la granularità richiesta**: scenari #15/#16 di sezione 18 (pausa abbonamento breve/lunga) — bloccati dal gap noto `effective_active_days`/`checkin_available_at` mai popolati da alcuna funzione (sezione 12.3 di questo documento, mai implementata: nessun trigger/job su `user_subscriptions`). Lo scenario "scaduto→rinnovato" **è stato** testato con successo (BUG-059), ma la metrica fine "giorni effettivamente attivi durante la pausa" resta non implementata.
- **Non eseguito**: nessun clic reale in browser/app in questo ambiente (nessun tool di automazione disponibile) — checklist completa in `docs/TODO_NEXT.md`.

**Blocco 2 "Programmi automatici" dichiarato completo con questa sessione (sotto-blocchi 2.0-2.7 tutti implementati e testati). Non si procede al Blocco 3**, come richiesto esplicitamente due volte dall'utente.
