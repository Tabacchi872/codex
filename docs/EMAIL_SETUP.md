...
- Il link nell'email di Supabase riporta l'utente su `redirectTo` (vedi sotto), ma il **formato** dipende da come e' scritto il template "Reset password" sul progetto (Authentication → Email Templates):
  - **`{{ .ConfirmationURL }}`** (pattern classico): il link passa prima da `{project}.supabase.co/auth/v1/verify?...&type=recovery&redirect_to=...`, che verifica il token e reindirizza su `redirectTo` con un frammento `#access_token=...&type=recovery` nell'URL. Gestito **automaticamente** da `detectSessionInUrl`.
  - **`{{ .TokenHash }}`** (pattern piu' recente, redirect diretto nell'app senza passare dal dominio Supabase): il link punta direttamente su `redirectTo` con `?token_hash=...&type=recovery` nella QUERY STRING. Questo formato **non viene mai gestito automaticamente** da `detectSessionInUrl` — richiede una chiamata esplicita a `supabase.auth.verifyOtp({ type: 'recovery', token_hash })`, aggiunta in `reset-password-screen.tsx` (fix 2026-07-12, BUG-012): senza, l'utente restava bloccato senza una sessione di recovery e senza capire perche'.
  - **Verifica quale dei due usa il tuo progetto**: apri Authentication → Email Templates → "Reset password" e controlla se il link nel corpo usa `{{ .ConfirmationURL }}` o `{{ .TokenHash }}` (o entrambi in URL diversi). Il codice ora gestisce entrambi i formati, quindi non serve cambiare il template per far funzionare il flusso — ma se il template e' stato personalizzato in modo non standard (nessuna delle due variabili), il link non conterra' ne' l'uno ne' l'altro e il flusso non potra' funzionare.
- `mobile/src/lib/supabase.ts`: `detectSessionInUrl` e' ora attivo solo su web (`Platform.OS === 'web'`) — legge il frammento `#access_token=...` (formato 1) e stabilisce automaticamente una sessione di recovery prima che la UI monti. Su nativo resta disattivato (deep link nativo non gestito in questa fase, vedi sotto).
- Route `/reimposta-password` (`mobile/src/components/reset-password-screen.tsx`): legge prima `?token_hash=...&type=recovery` dalla query string (formato 2, tramite `useLocalSearchParams`) e se presente chiama `verifyOtp` esplicitamente; altrimenti aspetta che compaia la sessione stabilita automaticamente da `detectSessionInUrl` (formato 1, evento `PASSWORD_RECOVERY` o `getSession()`), poi mostra il form nuova password + conferma. Se non arriva nessuna sessione entro ~1.5s, mostra "Link non valido o scaduto" con pulsante per richiederne uno nuovo.
- **Importante — causa piu' comune se il link porta al login invece che al form di reset**: la "Redirect URLs" del progetto Supabase (Authentication → URL Configuration) NON include l'origin/porta effettivamente in uso. In quel caso Supabase **ignora silenziosamente** il `redirectTo` richiesto dall'app e usa la "Site URL" di default (di solito la radice `/`) — il link porta quindi fuori da `/reimposta-password`, e con nessuna sessione locale attiva l'app mostra il login. Vedi la sezione "Redirect URL" sotto.
