...
- [ ] **Verifica reale del collegamento Supabase Fase 1 (priorità alta, mai fatta):**
  1. Creare un progetto Supabase reale, eseguire `docs/SUPABASE_SCHEMA.sql` nel SQL editor.
  2. In Authentication → Providers → Email, disattivare "Confirm email" (necessario perché il flusso Fase 1 scriva le righe aggiuntive subito dopo la registrazione — vedi `docs/SUPABASE_SCHEMA.md`/`docs/EMAIL_SETUP.md`).
  3. Copiare `mobile/.env.example` in `mobile/.env` e compilare `EXPO_PUBLIC_SUPABASE_URL`/`EXPO_PUBLIC_SUPABASE_ANON_KEY`, riavviare `expo start --web`.
  4. Registrare un coach reale → verificare in Supabase (Table editor) che esistano le righe in `profiles`, `coach_profiles`, `billing_profiles`, `registration_codes` con lo stesso codice mostrato in UI.
  5. Registrare un cliente con quel codice → verificare `profiles`, `client_profiles`, `coach_clients`, e che `registration_codes.used_count` sia salito a 1.
  6. Provare un codice inesistente e un codice di un coach con `coach_profiles.billing_status` impostato manualmente a `blocked`/`past_due`/`canceled` → verificare che la registrazione cliente venga bloccata con il messaggio corretto.
  7. Fare logout e login con le credenziali appena create (email/password) → verificare che il login reale funzioni e che il ruolo (coach/cliente) porti alla home corretta.
  8. Verificare che con `mobile/.env` assente/vuoto l'app si comporti esattamente come prima (login/registrazione locale, nessun errore Supabase in console).
