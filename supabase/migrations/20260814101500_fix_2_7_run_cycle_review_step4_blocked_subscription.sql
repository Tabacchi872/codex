-- Fix (scoperto nella campagna di test 2.7, ultimo anello della catena sul
-- rinnovo abbonamento — vedi le tre migration precedenti sullo stesso
-- scenario): dopo il ripristino, con lo stato di nuovo 'active' e un nuovo
-- check-in sottomesso, la query generale di idempotenza dello STEP 4
-- (quella per i cicli NON in stato terminale) selezionava comunque la
-- VECCHIA riga 'blocked_subscription' come "ultima review non-
-- insufficient_data" e la restituiva come se fosse ancora la decisione
-- valida — bloccando per sempre la nuova revisione con lo stesso esito
-- cache gia' visto prima del rinnovo. Fix: 'blocked_subscription' esclusa
-- da questa select esattamente come 'insufficient_data' — e' un blocco
-- temporaneo per abbonamento, mai una decisione definitiva sul programma
-- (la controparte in submit_monthly_checkin e' gia' stata corretta nella
-- migration precedente con lo stesso principio).
...
begin
  ...
  -- FIX (questa migration): 'blocked_subscription' esclusa qui esattamente
  -- come 'insufficient_data' — entrambe sono blocchi temporanei, mai una
  -- decisione definitiva sul programma.
  select * into v_review from public.client_cycle_reviews
  where client_cycle_reviews.cycle_id = v_cycle.id
    and client_cycle_reviews.decision <> all(array['insufficient_data','blocked_subscription']::text[])
    and not exists (
