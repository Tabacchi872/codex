...
-- superadmin_force_cycle_decision() con un account sintetico (dopo il fix
-- precedente sullo stato): client_cycle_reviews.origin e
-- client_cycle_exercise_transitions.origin hanno CHECK con vocabolari
-- diversi, mai allineati tra loro nello schema pre-esistente dal 2.1
-- (`client_cycle_reviews_origin_check`: 'automatic'/'superadmin';
-- `client_cycle_exercise_transitions_origin_check`:
-- 'automatic'/'superadmin_override'). La funzione li aveva scambiati:
-- 'superadmin_override' nell'insert su client_cycle_reviews (violazione
-- CHECK, bug live confermato) e 'superadmin' nell'insert su
-- client_cycle_exercise_transitions (avrebbe violato lo stesso CHECK alla
-- prima riga se la prima insert non avesse gia' fallito prima). Corretto
-- usando il valore giusto per ciascuna tabella. Nessun'altra modifica.
...
begin
  ...
  -- FIX (questa migration): origin='superadmin' (non 'superadmin_override')
  -- per client_cycle_reviews.
  ...
  loop
    ...
    -- FIX (questa migration): origin='superadmin_override' (non
    -- 'superadmin') per client_cycle_exercise_transitions.
