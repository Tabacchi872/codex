-- Fix isolato: le funzioni con `returns table (... cycle_id uuid ...)`
-- (check_cycle_review_eligibility) e `returns table (... cycle_id uuid,
-- decision text ...)` (run_cycle_review) creano parametri OUT che
-- diventano variabili PL/pgSQL con quel nome nell'intero corpo funzione.
-- Alcune query interne referenziavano `cycle_id`/`decision` senza
-- qualificare la tabella, generando un errore reale "column reference is
-- ambiguous" a runtime (mai emerso nella validazione sintattica via
-- BEGIN/ROLLBACK, che verifica solo che CREATE FUNCTION sia sintatticamente
-- valido, non che ogni query interna sia semanticamente risolvibile) —
-- scoperto eseguendo per la prima volta check_cycle_review_eligibility con
-- un account sintetico. Corretto qualificando esplicitamente ogni
-- riferimento a `cycle_id`/`decision` con il nome della tabella
-- (client_cycle_reviews/client_monthly_checkins) in tutti i punti coinvolti.
-- Nessuna modifica di logica: stesso comportamento, solo riferimenti
-- di colonna resi non ambigui.
...
begin
  ...
  select exists(
    select 1 from public.client_cycle_reviews where client_cycle_reviews.cycle_id = v_cycle.id and client_cycle_reviews.decision <> 'insufficient_data'
