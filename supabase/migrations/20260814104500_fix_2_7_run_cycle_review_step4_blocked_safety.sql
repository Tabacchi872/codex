...
-- migration precedente ma per 'blocked_safety'): quando un Superadmin
-- risolve una revisione di sicurezza con 'approve_continue'
-- (superadmin_resolve_safety_review), il ciclo riprende riusando LO STESSO
-- cycle_id — la vecchia riga 'blocked_safety' restava pero' a sistema e la
-- query generale di idempotenza dello STEP 4 la trovava ancora come
-- "ultima review non-insufficient_data/non-blocked_subscription",
-- restituendola per sempre come esito cache anche al successivo ciclo di
-- revisione naturale (nuovo check-in, nuova scadenza). Fix:
-- 'blocked_safety' esclusa da questa select esattamente come
-- 'insufficient_data'/'blocked_subscription' — e' un blocco temporaneo,
-- mai una decisione definitiva sul programma, una volta che il ciclo e'
-- gia' tornato aperto (submit_monthly_checkin corretto con lo stesso
-- principio nella migration precedente).
