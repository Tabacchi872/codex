-- fix: run_cycle_review, cambio strutturale di modello (STEP 16a) puo'
-- scegliere un nuovo template compatibile in generale (tramite
-- _match_auto_template, che verifica goal/livello/luogo/attrezzatura) ma
-- INCOMPATIBILE con le esclusioni personali di QUESTO cliente specifico
-- (client_excluded_exercises) — _match_auto_template non le considera mai,
-- per costruzione (sono per-cliente, non per-template).
--
-- Comportamento attuale gia' verificato leggendo il codice (non e' il "copia
-- 1:1 senza alcun filtro" originariamente ipotizzato): STEP 16a copia tutti i
-- giorni del nuovo template, POI cancella gli esercizi esclusi, POI elimina
-- ogni piano/giorno rimasto vuoto, e se TUTTI i giorni risultano vuoti fa
-- rollback completo a 'pending_template' con notifica Superadmin — questa
-- parte e' gia' corretta e resta invariata.
--
-- Il gap reale: se SOLO ALCUNI giorni (non tutti) restano vuoti dopo aver
-- rimosso gli esercizi esclusi, quei giorni vengono scartati silenziosamente
-- e il ciclo prosegue con MENO giorni di quanti il cliente ha dichiarato
-- disponibili — nessuna notifica, nessuna review, nessun pending_template —
-- una riduzione silenziosa del programma, diversa dalla semantica gia'
-- prevista in assign_initial_auto_program (che rifiuta l'INTERO template in
-- anticipo, prima di scrivere qualunque scheda, se anche un solo giorno
-- risulterebbe incompatibile dopo le esclusioni — vedi
-- 20260816100000:274-322).
--
-- Fix: nuovo helper _template_compatible_with_client_exclusions() (stessa
-- logica di "ogni giorno ha almeno un esercizio non escluso" gia' usata in
-- assign_initial_auto_program, riusata qui invece di duplicarla in modo
-- divergente), usato per SCARTARE upfront un candidato di
-- _match_auto_template che le esclusioni personali del cliente
-- svuoterebbero anche solo parzialmente — PRIMA di creare qualunque scheda,
-- non dopo. Nei due punti in cui STEP 12/13 gia' gestiscono "nessun
-- candidato trovato" (pending_template per il cambio strutturale; resta sul
-- template attuale per la regressione severa), un candidato reso
-- incompatibile dalle esclusioni viene ora trattato ESATTAMENTE come
-- "nessun candidato trovato" — stessa semantica gia' prevista, nessun nuovo
-- stato/percorso introdotto.
--
-- NON tocca: _copy_template_days_to_plans (condivisa con
-- assign_workout_template_to_client, percorso Coach -> Cliente, fuori
-- perimetro del motore automatico e non modificata), _match_auto_template
-- (nessuna nuova firma, nessun nuovo parametro), il percorso STEP 16b
-- (stesso template, nessun cambio strutturale — gia' filtra le esclusioni
-- correttamente tramite exercise_alternatives, invariato).
--
-- Il resto del corpo di run_cycle_review sotto e' RIPRODOTTO IDENTICO alla
-- versione attualmente attiva (20260816090000): unica differenza le due
-- righe segnalate esplicitamente "FIX (questa migration)" nei commenti STEP
-- 12 e STEP 13.

-- ============================================================================
-- Helper: un template resta interamente utilizzabile per un cliente specifico
-- dopo aver rimosso le sue esclusioni personali? (ogni giorno del template
-- deve avere almeno un esercizio non escluso). Non verifica di nuovo
-- livello/attrezzatura/luogo: _match_auto_template garantisce gia' quella
-- compatibilita' generica per l'intero template prima di restituirlo come
-- candidato — qui si controlla solo l'ulteriore restringimento dovuto alle
-- esclusioni PERSONALI del cliente, che _match_auto_template non conosce.
-- ============================================================================
create or replace function public._template_compatible_with_client_exclusions(
  p_template_id uuid,
  p_client_id uuid
)
returns boolean
...
as $function$
  -- Un template con ZERO giorni non e' "utilizzabile": senza il primo
  -- exists esplicito, un template senza righe in workout_template_days
  -- soddisferebbe vacuamente il "not exists (giorno vuoto)" sotto e
  -- risulterebbe erroneamente compatibile=true. Nei call-site reali
  -- (run_cycle_review) questo caso non si presenta mai (_match_auto_template
  -- accetta solo candidati con day_count = p_days, sempre >= 1), ma la
  -- funzione deve restare corretta anche isolata dal suo unico chiamante
  -- attuale.
  select exists (
    select 1 from public.workout_template_days wtd where wtd.template_id = p_template_id
  )
  and not exists (
    select 1
    from public.workout_template_days wtd
    where wtd.template_id = p_template_id
      and not exists (
        select 1
        from public.workout_template_exercises wte
        where wte.template_day_id = wtd.id
          and wte.exercise_id not in (
            select exercise_id from public.client_excluded_exercises
...
revoke all on function public._template_compatible_with_client_exclusions(uuid, uuid) from public, anon, authenticated;

-- Sanity check side-effect-free: un template reale con un client_id che non
-- ha (e non puo' avere, e' casuale) alcuna esclusione deve risultare
-- compatibile. Verifica solo che la funzione compili ed esegua senza errori
-- SQL, nessuna scrittura.
do $$
declare
  v_any_template_id uuid;
  v_result boolean;
begin
  select wt.id into v_any_template_id
  from public.workout_templates wt
  where wt.is_system and exists (select 1 from public.workout_template_days wtd where wtd.template_id = wt.id)
  limit 1;
  if v_any_template_id is not null then
    v_result := public._template_compatible_with_client_exclusions(v_any_template_id, gen_random_uuid());
    if v_result is distinct from true then
      raise exception 'TEMPLATE_EXCLUSION_HELPER_SANITY_CHECK_FAILED: atteso true per un client_id senza esclusioni, ottenuto %', v_result;
    end if;
    raise notice 'TEMPLATE_EXCLUSION_HELPER_SANITY_CHECK_OK template_id=%', v_any_template_id;
  end if;
end
$$;

-- ============================================================================
-- run_cycle_review — stesso corpo della versione attiva (20260816090000),
-- con le due sole modifiche segnalate "FIX (questa migration)" negli STEP
-- 12 e 13 sotto.
-- ============================================================================
...
begin
  ...
  if v_template.id is not null then
    ...
    if v_structural_change then
      ...
      -- FIX (questa migration): un candidato trovato da _match_auto_template
      -- (compatibile in generale) ma svuotato anche solo parzialmente dalle
      -- esclusioni PERSONALI di questo cliente viene trattato come "nessun
      -- candidato" — stessa semantica di assign_initial_auto_program, mai
      -- una riduzione silenziosa dei giorni del programma.
      if v_candidate_template_id is null or not public._template_compatible_with_client_exclusions(v_candidate_template_id, v_client_id) then
  ...
  if v_decision = 'regress' and v_severe_regress and v_next_template_id = v_cycle.template_id and v_template.id is not null then
    ...
    if v_lower_level is not null then
      ...
      -- FIX (questa migration): stesso principio dello STEP 12 — un
      -- candidato svuotato anche solo parzialmente dalle esclusioni
      -- personali del cliente non viene adottato; si ricade sul
      -- comportamento gia' esistente (nessun candidato trovato -> resta sul
      -- template attuale, regressione applicata in-place allo STEP 16b).
      if v_candidate_template_id is not null and public._template_compatible_with_client_exclusions(v_candidate_template_id, v_client_id) then
        v_next_template_id := v_candidate_template_id;
      end if;
    end if;
  end if;
  ...
  -- ---- STEP 14.5: chiudi ORA il ciclo precedente -----------------------
  update public.client_program_cycles
