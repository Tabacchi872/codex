-- Fix (scoperto nella campagna di test 2.7, stessa classe di bug della
-- migration precedente ma per il percorso 'blocked_safety'): quando un
-- Superadmin risolve una revisione di sicurezza con l'azione
-- 'approve_continue' (superadmin_resolve_safety_review), il ciclo riprende
-- riusando LO STESSO cycle_id (a differenza di 'approve_new_template' o
-- di qualunque risoluzione pending_template, che creano sempre un nuovo
-- ciclo) — la vecchia riga 'blocked_safety' resta pero' a sistema.
-- submit_monthly_checkin la trovava ancora con l'ALREADY_REVIEWED check e
-- rifiutava il check-in del ciclo successivo naturale, bloccando per
-- sempre un cliente il cui blocco di sicurezza era gia' stato risolto.
-- Fix: 'blocked_safety' esce dal controllo esattamente come
-- 'insufficient_data'/'blocked_subscription' — e' un blocco temporaneo,
-- mai una decisione definitiva sul programma, quando il ciclo e' tornato
-- in uno stato che permette la sottomissione di un check-in.
...
begin
  ...
  -- FIX (questa migration): 'blocked_safety' esclusa anche qui, stesso
  -- principio di 'insufficient_data'/'blocked_subscription' — non e' una
  -- decisione definitiva sul programma quando il ciclo e' gia' tornato in
  -- uno stato che ammette un nuovo check-in (il gate sullo stato sopra
  -- garantisce che un ciclo con una vera decisione definitiva non possa
  -- trovarsi qui).
  if exists (
    select 1 from public.client_cycle_reviews
    where cycle_id = p_cycle_id and decision <> all(array['insufficient_data','blocked_subscription','blocked_safety']::text[])
  ) then
