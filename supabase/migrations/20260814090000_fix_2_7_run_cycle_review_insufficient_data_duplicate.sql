-- Fix (scoperto nella campagna di test 2.7, con token REST reali e due
-- chiamate concorrenti reali su run_cycle_review): a differenza dei rami
-- 'blocked_safety' (STEP 8) e 'pending_template' (STEP 12/16a), il ramo
-- 'insufficient_data' (STEP 10) non aveva alcuna guardia "se esiste gia'
-- una review per lo stesso check-in, restituiscila invece di inserirne
-- una nuova": due chiamate ravvicinate sullo stesso check-in (due device,
-- retry dopo timeout) producevano due righe client_cycle_reviews
-- duplicate con decisione 'insufficient_data' (l'indice unico parziale
-- esclude esplicitamente questa decisione, quindi nulla la impediva).
-- pg_advisory_xact_lock + "for update" gia' presenti serializzano le
-- chiamate ma non le deduplicano: la seconda, dopo il lock, ripartiva da
-- zero e reinseriva. Fix: stessa idiom "if not found, insert" gia' usata
-- per blocked_safety/pending_template, chiave sul checkin_id (un
-- check-in resta lo stesso id finche' il cliente non lo modifica prima
-- della review, coerente con "editing consentito prima della review").
...
begin
  ...
  if v_metrics.sessions_completed < v_min_sessions or v_metrics.completion_ratio < v_min_completion_ratio or v_metrics.evaluable_ratio < v_min_exercise_data_ratio then
    -- FIX (questa migration): a differenza degli altri rami non-eleggibili,
    -- questo non aveva la guardia "se esiste gia' una review per questo
    -- identico check-in, restituiscila invece di reinserirla": due chiamate
    -- ravvicinate sullo stesso check-in (due device, retry dopo timeout)
    -- creavano righe 'insufficient_data' duplicate (l'indice unico
    -- parziale le esclude deliberatamente, per permettere un vero retry
    -- dopo che il cliente logga altre sessioni con un NUOVO check-in).
    select * into v_review from public.client_cycle_reviews
