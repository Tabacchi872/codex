-- Fix (scoperto nella campagna di test 2.7, rinnovo abbonamento con token
-- REST reali): un ciclo messo in 'paused_subscription' dallo STEP 6 non si
-- riprendeva mai automaticamente al rinnovo dell'abbonamento. Lo STEP 4
-- (idempotenza) intercettava qualunque review non-insufficient_data la cui
-- cycle.status non fosse 'pending_safety_review'/'pending_template' e
-- restituiva sempre la vecchia decisione cache ('already_reviewed'): un
-- cliente che rinnovava restava bloccato per sempre, senza intervento
-- Superadmin, nonostante l'abbonamento fosse di nuovo attivo.
-- Fix: 'paused_subscription' si comporta ora come gli altri due stati di
-- attesa (non e' un esito definitivo, va sempre ricontrollato) — la STEP 6
-- ricontrolla l'entitlement ad ogni chiamata: se e' tornato attivo, il
-- flusso prosegue normalmente (il ciclo esce da 'paused_subscription' allo
-- STEP 14.5 come qualunque altra decisione); se e' ancora inattivo, non
-- reinserisce una nuova review 'blocked_subscription' (stesso pattern
-- "if not found, insert" gia' usato per blocked_safety/pending_template/
-- insufficient_data) ma restituisce quella esistente, evitando duplicati.
-- resumed_at viene ora valorizzato (era dichiarato nello schema 2.1 ma mai
-- scritto da alcuna funzione).
...
begin
  ...
  order by reviewed_at desc limit 1;
  -- FIX (questa migration): 'paused_subscription' esce da questo
  -- short-circuit, esattamente come 'pending_safety_review'/'pending_template'
  -- gia' facevano — non e' un esito definitivo, l'entitlement va
  -- ricontrollato ad ogni chiamata (puo' cambiare in autonomia al rinnovo,
  -- a differenza degli altri due che aspettano sempre un intervento umano).
  if found and v_cycle.status <> any(array['pending_safety_review','pending_template','paused_subscription']::text[]) then
    return query select v_review.decision, v_cycle.id, v_review.next_cycle_id, v_review.decision_reason, true, 'already_reviewed'::text, v_review.metrics_snapshot;
    return;
  end if;
