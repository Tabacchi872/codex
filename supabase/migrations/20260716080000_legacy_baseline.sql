...
drop policy if exists coach_profiles_superadmin_all on public.coach_profiles;
create policy coach_profiles_superadmin_all on public.coach_profiles
  for all using (public.is_superadmin()) with check (public.is_superadmin());
drop policy if exists coach_profiles_owner_read_update on public.coach_profiles;
...
drop policy if exists billing_profiles_superadmin_all on public.billing_profiles;
create policy billing_profiles_superadmin_all on public.billing_profiles
  for all using (public.is_superadmin()) with check (public.is_superadmin());
drop policy if exists billing_profiles_coach_read_update_own on public.billing_profiles;
create policy billing_profiles_coach_read_update_own on public.billing_profiles
  for all using (coach_id = auth.uid()) with check (coach_id = auth.uid());

drop policy if exists client_profiles_superadmin_all on public.client_profiles;
create policy client_profiles_superadmin_all on public.client_profiles
  for all using (public.is_superadmin()) with check (public.is_superadmin());
drop policy if exists client_profiles_self_read_update on public.client_profiles;
...
drop policy if exists registration_codes_superadmin_all on public.registration_codes;
create policy registration_codes_superadmin_all on public.registration_codes
  for all using (public.is_superadmin()) with check (public.is_superadmin());
drop policy if exists registration_codes_coach_read_own on public.registration_codes;
create policy registration_codes_coach_read_own on public.registration_codes
  for select using (coach_id = auth.uid());
drop policy if exists registration_codes_coach_insert_own on public.registration_codes;
create policy registration_codes_coach_insert_own on public.registration_codes
  for insert with check (coach_id = auth.uid());
drop policy if exists registration_codes_coach_update_own on public.registration_codes;
...
drop policy if exists registration_codes_public_read_active on public.registration_codes;
...
drop policy if exists registration_codes_increment_usage on public.registration_codes;
...
drop policy if exists coach_clients_superadmin_all on public.coach_clients;
...
drop policy if exists plans_superadmin_write on public.plans;
create policy plans_superadmin_write on public.plans
  for all using (public.is_superadmin()) with check (public.is_superadmin());

drop policy if exists coach_billing_superadmin_all on public.coach_billing;
create policy coach_billing_superadmin_all on public.coach_billing
  for all using (public.is_superadmin()) with check (public.is_superadmin());
drop policy if exists coach_billing_owner_read on public.coach_billing;
create policy coach_billing_owner_read on public.coach_billing
  for select using (coach_id = auth.uid());

drop policy if exists payment_events_superadmin_all on public.payment_events;
create policy payment_events_superadmin_all on public.payment_events
  for all using (public.is_superadmin()) with check (public.is_superadmin());
drop policy if exists payment_events_coach_read on public.payment_events;
create policy payment_events_coach_read on public.payment_events
  for select using (coach_id = auth.uid());

drop policy if exists invoices_superadmin_all on public.invoices;
create policy invoices_superadmin_all on public.invoices
  for all using (public.is_superadmin()) with check (public.is_superadmin());
drop policy if exists invoices_coach_read_own on public.invoices;
create policy invoices_coach_read_own on public.invoices
  for select using (coach_id = auth.uid());

drop policy if exists invoice_items_superadmin_all on public.invoice_items;
create policy invoice_items_superadmin_all on public.invoice_items
  for all using (public.is_superadmin()) with check (public.is_superadmin());
drop policy if exists invoice_items_coach_read_own on public.invoice_items;
...
create policy subscriptions_superadmin_all on public.subscriptions
  for all using (public.is_superadmin()) with check (public.is_superadmin());
drop policy if exists subscriptions_coach_scope on public.subscriptions;
...
create policy messages_superadmin_all on public.messages
  for all using (public.is_superadmin()) with check (public.is_superadmin());
drop policy if exists messages_coach_scope on public.messages;
...
drop policy if exists admin_notifications_superadmin_all on public.admin_notifications;
create policy admin_notifications_superadmin_all on public.admin_notifications
  for all using (public.is_superadmin()) with check (public.is_superadmin());

drop policy if exists push_tokens_superadmin_all on public.push_tokens;
create policy push_tokens_superadmin_all on public.push_tokens
  for all using (public.is_superadmin()) with check (public.is_superadmin());
drop policy if exists push_tokens_owner_scope on public.push_tokens;
...
notify pgrst, 'reload schema';
-- ============================================================================
-- FitCoach Pro visual assets: client avatars + exercise image catalog
-- 2026-07-14. Idempotent: safe to rerun in Supabase SQL Editor.
-- ============================================================================
...
-- Existing column `profiles.avatar_url` is reused for the Storage object path
-- or an external URL. The mobile app resolves Storage paths through signed URLs.
...
create table if not exists public.exercise_image_catalog (
  catalog_id text primary key,
  exercise_id text,
  source text not null default 'fitcoach'
    check (source in ('fitcoach', 'ymove', 'custom', 'placeholder')),
  muscle_group text,
  image_storage_path text,
  placeholder_label text,
  placeholder_background text,
  placeholder_foreground text,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);

create index if not exists exercise_image_catalog_exercise_id_idx
  on public.exercise_image_catalog(exercise_id)
  where exercise_id is not null;

drop trigger if exists exercise_image_catalog_set_updated_at on public.exercise_image_catalog;
create trigger exercise_image_catalog_set_updated_at before update on public.exercise_image_catalog
for each row execute function public.set_updated_at();

alter table public.exercise_image_catalog enable row level security;

drop policy if exists exercise_image_catalog_read_authenticated on public.exercise_image_catalog;
create policy exercise_image_catalog_read_authenticated on public.exercise_image_catalog
for select
to authenticated
using (true);

drop policy if exists exercise_image_catalog_superadmin_all on public.exercise_image_catalog;
create policy exercise_image_catalog_superadmin_all on public.exercise_image_catalog
for all
to authenticated
