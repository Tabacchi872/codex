...
-- sotto-blocco 2.5 force_cycle_decision -> cancel_pending_review -> nuovo
-- tentativo, con un account sintetico: lo stesso difetto scoperto in
-- superadmin_force_cycle_decision esiste anche qui, nella STEP 4
-- (idempotenza). Dopo che un Superadmin annulla una review con
-- superadmin_cancel_pending_review, il ciclo precedente torna
-- 'review_pending' ma la vecchia review (mai cancellata: e' storico) resta
-- con decision <> 'insufficient_data' -- quindi la STEP 4 la trovava
-- sempre e restituiva per sempre lo stesso risultato 'already_reviewed'
-- stantio, impedendo qualunque nuova revisione automatica dello stesso
-- ciclo dopo un annullamento Superadmin. Corretto con lo stesso principio
-- gia' usato per force_cycle_decision: una review conta come "gia'
-- definitiva" solo se il suo next_cycle_id (quando presente) NON punta a
-- un ciclo 'cancelled' (l'unico modo in cui questo accade e'
-- superadmin_cancel_pending_review). Nessun'altra modifica: stesso
-- identico corpo della funzione applicata in 20260811090000.
