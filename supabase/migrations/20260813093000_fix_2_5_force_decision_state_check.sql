...
-- superadmin_force_cycle_decision() con un account sintetico: il controllo
-- di stato usava `v_cycle.status <> any(array[...])`, che in SQL e' vero
-- se lo stato differisce da ALMENO UNO degli elementi dell'array -- quasi
-- sempre vero, non "lo stato non e' tra quelli ammessi" (per quello serve
-- `<> all(array[...])`, stesso identico pattern gia' corretto usato da
-- submit_monthly_checkin per lo stesso tipo di controllo). Bug live
-- confermato: un ciclo realmente in stato 'active' (ammesso) veniva
-- rifiutato con INVALID_STATE alla prima chiamata reale della funzione.
-- Corretto sostituendo `<> any` con `<> all`. Nessun'altra modifica.
...
begin
  ...
  -- FIX (questa migration): `<> all`, non `<> any`.
