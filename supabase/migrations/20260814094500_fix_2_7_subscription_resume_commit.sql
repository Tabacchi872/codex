...
-- precedente sul rinnovo abbonamento, dopo aver gia' corretto il bug
-- `<> any`/`<> all` in questa stessa funzione): il ripristino di
-- 'paused_subscription' introdotto in 20260814091500 impostava
-- `v_cycle.status := 'active'` solo in memoria (variabile locale) e
-- proseguiva nella STESSA chiamata fino allo STEP 7, che senza un
-- check-in gia' presente solleva `raise exception 'CHECKIN_REQUIRED'`.
-- In PL/pgSQL un'eccezione non catturata annulla l'INTERA transazione
-- della funzione, incluso l'UPDATE di resumed_at/status fatto poco prima:
-- il ripristino non veniva mai davvero scritto sul DB. Riprova:
-- submit_monthly_checkin rifiuta esplicitamente lo stato
-- 'paused_subscription' (INVALID_CYCLE_STATE), quindi il cliente restava
-- bloccato in un vicolo cieco (il ciclo non usciva mai da
-- 'paused_subscription' perche' il check-in richiesto per farlo uscire non
-- poteva essere inviato mentre lo stato era ancora 'paused_subscription').
-- Fix: il ripristino ora fa l'UPDATE e RESTITUISCE SUBITO (result_code
-- 'resumed', blocked=false, nessuna eccezione, nessuna riga di review
-- inserita: non e' una decisione sul programma, e' un ripristino di
-- stato) — la transazione si chiude qui e l'UPDATE viene commesso
-- davvero. Il chiamante rifara' la chiamata (dallo stesso identico punto
-- da cui gia' oggi si richiama run_cycle_review dopo un check-in): se un
-- check-in non esiste ancora, submit_monthly_checkin ora lo accetta
-- (stato non piu' 'paused_subscription'), e la chiamata successiva a
-- run_cycle_review procede come di consueto.
