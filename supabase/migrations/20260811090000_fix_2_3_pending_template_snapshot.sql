-- Fix isolato trovato eseguendo per la prima volta run_cycle_review() con un
-- account sintetico attraverso il vero percorso "cambio struttura del
-- modello -> tutti i giorni vuoti dopo le esclusioni" (STEP 16a), mai
-- eseguito dal vivo prima d'ora (il sotto-blocco 2.3 lo aveva verificato
-- solo per lettura del codice). Due difetti trovati, stessa causa radice
-- (il ramo STEP 16a non allinea lo stato finale a quanto fa il ramo
-- gemello di STEP 12 per lo stesso identico esito `pending_template`):
--
-- 1) `metrics_snapshot` non veniva mai valorizzato in questo ramo (restava
--    vuoto): a differenza del ramo STEP 12 (nessun modello trovato affatto),
--    che documenta modello richiesto/luogo/obiettivo, il ramo STEP 16a
--    (modello trovato ma svuotato dalle esclusioni cliente) non lasciava
--    alcuna traccia strutturata della causa per il Superadmin -- solo la
--    stringa `decision_reason`. Corretto: stesso pattern di jsonb del ramo
--    STEP 12, con l'aggiunta del modello candidato tentato e degli id
--    esercizio esclusi realmente responsabili dello svuotamento.
-- 2) `replaced_at` restava valorizzato (impostato da STEP 14.5, quando lo
--    stato finale sembrava ancora "sostituito da un nuovo ciclo") anche
--    dopo che STEP 16a annulla la creazione del nuovo ciclo e riporta lo
--    stato del vecchio ciclo a `pending_template`: un Superadmin che legge
--    la riga vedrebbe un `replaced_at` popolato su un ciclo che non e' mai
--    stato davvero sostituito (nessun ciclo successivo esiste). Corretto:
--    azzerato insieme al cambio di stato, stesso costrutto SQL.
--
-- Nessuna modifica di logica decisionale: stesso identico corpo della
-- funzione applicata in 20260810100000, solo i due punti sopra.
...
begin
  ...
  if v_next_template_id is distinct from v_cycle.template_id and v_next_template_id is not null then
    ...
    if array_length(v_new_plan_ids, 1) is null then
      ...
      -- FIX (questa migration): azzera anche replaced_at/completed_at, mai
      -- davvero validi qui (nessun ciclo successivo esiste al ritorno).
      update public.client_program_cycles
      set status = 'pending_template', replaced_at = null, completed_at = null
      where id = v_cycle.id;

      -- FIX (questa migration): valorizza metrics_snapshot con la stessa
      -- ricchezza del ramo gemello di STEP 12 (modello richiesto/luogo,
      -- modello candidato tentato, id esercizio esclusi responsabili dello
      -- svuotamento) invece di lasciarlo vuoto -- il Superadmin deve poter
      -- vedere la causa strutturata, non solo la frase in decision_reason.
