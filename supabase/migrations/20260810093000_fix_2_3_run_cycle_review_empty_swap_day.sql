-- Fix isolato su run_cycle_review (introdotta in
-- 20260810090000_block2_cycle_review_engine.sql, gia' applicata: questa
-- migrazione NON la modifica, la ridefinisce con CREATE OR REPLACE).
--
-- Difetto trovato in revisione: il ramo di cambio strutturale di modello
-- (STEP 16a, quando il check-in segnala un luogo/attrezzatura/obiettivo
-- diverso e viene trovato un nuovo modello compatibile) copiava i giorni del
-- nuovo modello e rimuoveva gli esercizi esclusi dal cliente, ma — a
-- differenza di assign_initial_auto_program, che ha questo stesso controllo
-- — non verificava se un giorno restasse senza alcun esercizio dopo la
-- rimozione. Se gli esercizi esclusi dal cliente coprono per intero un
-- giorno del nuovo modello, quel giorno sarebbe stato creato come scheda
-- vuota: un programma "silenziosamente rotto", la stessa categoria di
-- difetto che le regole del progetto vietano per l'interfaccia, qui a
-- livello di dati.
--
-- Correzione: dopo la rimozione degli esercizi esclusi, ogni giorno rimasto
-- vuoto viene eliminato (come nell'assegnazione iniziale); se TUTTI i giorni
-- del nuovo modello risultano vuoti, l'esito diventa pending_template
-- (nessun ciclo con una scheda vuota): il ciclo successivo appena creato
-- allo STEP 15 — non ancora referenziato da alcuna riga figlia in questo
-- ramo — viene eliminato, e la review gia' inserita allo STEP 14 viene
-- riconvertita in una decisione pending_template invece di crearne una
-- seconda.
...
begin
  ...
  if v_next_template_id is distinct from v_cycle.template_id and v_next_template_id is not null then
    ...
    -- Nessun giorno deve restare vuoto in silenzio dopo l'esclusione (stessa
    -- garanzia di assign_initial_auto_program, sezione 2): se un giorno del
    -- NUOVO modello risulta composto solo da esercizi esclusi dal cliente,
    -- quel giorno viene rimosso; se TUTTI i giorni risultano vuoti, lo
    -- scambio di modello fallisce e l'esito diventa pending_template (mai
    -- un ciclo con una scheda vuota).
    v_swap_kept_ids := '{}';
    ...
    if array_length(v_new_plan_ids, 1) is null then
      -- Fallback a pending_template: il ciclo appena creato allo STEP 15 non
      -- e' ancora referenziato da alcuna riga figlia (nessuna
      -- client_program_cycle_plans inserita finora in questo ramo), quindi
      -- puo' essere eliminato senza lasciare orfani; la review gia' inserita
      -- allo STEP 14 viene riconvertita in una decisione pending_template
...
-- CREATE OR REPLACE non tocca gli EXECUTE gia' concessi/revocati; nessuna
-- modifica ai grants e' necessaria qui (gia' corretti dalla migrazione
-- 20260810091500: solo authenticated + service_role, mai anon/PUBLIC).
