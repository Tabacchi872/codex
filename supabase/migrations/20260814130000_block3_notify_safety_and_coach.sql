-- fix: Blocco 3 (notifiche) — completa 2 lacune reali trovate implementando
-- il centro notifiche: il cliente non veniva MAI avvisato quando il proprio
-- ciclo entra in pending_safety_review (solo il superadmin riceveva
-- 'review_blocked_safety') — la richiesta esplicita del Blocco 3 elenca
-- "sicurezza" tra le categorie di notifica CLIENTE. Aggiunta una notifica
-- gemella per il cliente in entrambi i punti che possono generare questo
-- blocco (STEP 8: prima volta dopo un check-in; ramo di rientro quando il
-- ciclo e' gia' in pending_safety_review), con lo STESSO dedup_key in
-- entrambi (mai due notifiche per lo stesso episodio di blocco).
...
begin
  ...
  if v_cycle.status = 'pending_safety_review' then
    ...
    if not found then
      ...
      -- FIX (questa migration): notifica gemella per il cliente, mai
      -- inviata prima d'ora in questo ramo di rientro.
      insert into public.app_notifications(recipient_id, recipient_role, type, title, body, data, dedup_key)
      values (
        v_client_id, 'cliente', 'review_blocked_safety_client', 'Il tuo programma e'' in pausa per sicurezza',
        'Il nostro team sta verificando la tua situazione prima di proseguire con il programma. Ti contatteremo appena possibile.',
        jsonb_build_object('cycle_id', v_cycle.id),
        'review_blocked_safety_client:' || v_cycle.id::text
      )
      on conflict do nothing;
    end if;
  ...
  if v_checkin.has_pain_or_limitation or v_checkin.requires_professional_supervision then
    ...
    -- FIX (questa migration): notifica gemella per il cliente, mai
    -- inviata prima d'ora — solo il superadmin veniva informato.
    insert into public.app_notifications(recipient_id, recipient_role, type, title, body, data, dedup_key)
    values (
      v_client_id, 'cliente', 'review_blocked_safety_client', 'Il tuo programma e'' in pausa per sicurezza',
      'Il nostro team sta verificando la tua situazione prima di proseguire con il programma. Ti contatteremo appena possibile.',
...
-- fix: Blocco 3 — il cliente non veniva MAI avvisato quando riceveva un
-- coach (passaggio self-guided -> coach-guided, sotto-blocco 2.4): il
-- trigger _handle_client_coach_assignment scriveva solo la review
-- 'manual_review' di audit, nessuna riga in app_notifications. La
-- richiesta esplicita del Blocco 3 elenca "assegnazione coach" tra le
-- categorie di notifica CLIENTE.
...
begin
  ...
  -- FIX (questa migration): notifica cliente, mai inviata prima d'ora.
  insert into public.app_notifications(recipient_id, recipient_role, type, title, body, data, dedup_key)
  values (
    new.client_id, 'cliente', 'auto_program_coach_assigned', 'Ti e'' stato assegnato un coach',
    'Da ora la gestione del tuo programma passa al tuo coach: il programma automatico precedente resta consultabile come storico.',
    jsonb_build_object('cycle_id', v_cycle.id, 'coach_id', new.coach_id),
    'auto_program_coach_assigned:' || v_cycle.id::text
  )
  on conflict do nothing;
