-- Fix isolato: run_cycle_review inseriva il nuovo ciclo (status='active')
-- ALLO STEP 15, mentre il ciclo precedente veniva chiuso solo allo STEP 17
-- (molto piu' avanti, dopo la copia dei piani). Con entrambi i cicli
-- 'active' simultaneamente per lo stesso client_id, l'INSERT del nuovo
-- ciclo viola l'indice unico parziale
-- `client_program_cycles_one_current_per_client_idx` ("un solo ciclo
-- corrente per cliente"), scoperto eseguendo per la prima volta
-- run_cycle_review con un account sintetico (mai emerso nella validazione
-- sintattica). Corretto anticipando la chiusura del ciclo precedente subito
-- dopo la creazione della review (quando lo stato finale del vecchio ciclo
-- e' gia' interamente noto: 'replaced' se il modello e' cambiato, altrimenti
-- 'completed'), PRIMA di inserire il nuovo ciclo. Nessuna modifica di
-- logica decisionale: solo l'ordine delle scritture.
