...
import { getClientPlans, getSessionDayLabel, getSessionWeekLabel, getWorkoutCounter } from '@/lib/workout-progress';
...
export default function ClienteDettaglioScreen() {
  ...
  return (
    <AppScreen contentStyle={styles.content}>
      <BackHeader title={clientFullName(cliente)} fallbackHref="/clienti" />
      ...
      <AppCard style={styles.heroCard}>
        <View style={styles.heroTopRow}>
          <UserAvatar
            ...
            preset={cliente.avatarPreset}
            size={74}
          />
          <View style={styles.heroText}>
            <Text style={[styles.heroEyebrow, { color: colors.moss }]}>SCHEDA CLIENTE</Text>
            <Text style={[styles.heroName, { color: colors.ink }]} numberOfLines={2}>
              {clientFullName(cliente)}
            </Text>
            <Text style={[styles.smallText, { color: colors.inkSoft }]} numberOfLines={2}>
              {cliente.goal || 'Obiettivo non specificato'}
            </Text>
          </View>
        </View>
        <View style={styles.heroStatsRow}>
          <View style={styles.heroMetric}>
            <Text style={[styles.metricLabel, { color: colors.inkSoft }]}>Workout</Text>
          ...
          <View
            style={[
              styles.progressFill,
              {
                ...
                width: `${workoutCounter.total > 0 ? Math.min(workoutCounter.completed / workoutCounter.total, 1) * 100 : 0}%`,
              },
            ]}
          />
        </View>
