...
export async function listCoachClientsForCurrentUser(): Promise<CoachClientsResult> {
  ...
  const [profilesRes, clientProfilesRes] = await Promise.all([
    supabase.from('profiles').select('id,full_name,email,phone,created_at').in('id', clientIds),
  ...
  const clients = uniqueLinks
    .map((link) => {
      ...
      return mapCoachClient(link, profile, clientProfile, coachId);
    })
...
function mapCoachClient(link: CoachClientRow, profile: ProfileRow, clientProfile: ClientProfileRow | undefined, coachId: string): Client {
